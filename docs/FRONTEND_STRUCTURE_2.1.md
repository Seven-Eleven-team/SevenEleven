...
└── 📂 admin
    ...
    ├── 📄 admin_users.jsp                     (회원 목록 / 검색 / 상태 관리)
    ├── 📄 admin_user_detail.jsp               (회원 상세)
    ├── 📄 admin_user_status.jsp               (회원 정지 / 탈퇴 처리)
    ├── 📄 admin_audit.jsp                     (회원 활동 로그 조회)
    ├── 📄 admin_terms.jsp                     (약관 목록 / 사용 여부 관리)
    ├── 📄 admin_terms_form.jsp                (약관 등록 / 수정)
    ├── 📄 admin_ott_sellers.jsp               (OTT 판매자 계정 관리)
    ├── 📄 admin_ott_seller_detail.jsp         (판매자 확인 / 승인 / 거절 사유)
    ├── 📄 admin_ott_purchases.jsp             (OTT 구매 요청 관리)
    ├── 📄 admin_ott_purchase_detail.jsp       (구매 승인 / 미처리 / 판매 삭제)
    ├── 📄 admin_support.jsp                   (고객센터 전체 관리)
    ├── 📄 admin_inquiries.jsp                 (문의 관리)
    ├── 📄 admin_faq.jsp                       (FAQ 관리)
    ├── 📄 admin_notification.jsp              (이메일 알림 로그 목록)
    ├── 📄 admin_notification_detail.jsp       (알림 상세 / 실패 사유)
