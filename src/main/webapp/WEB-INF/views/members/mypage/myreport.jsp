<%-- 경로: src/main/webapp/WEB-INF/views/members/mypage/myreport.jsp --%>
...
<div class="container">
    <aside class="sidebar">
        <ul>
            ...
            <li class="active">
                <a href="${pageContext.request.contextPath}/members/mypage/myreport">내 신고 목록</a>
            </li>
    ...
    <main class="report-main">
        ...
        <section class="report-card">
            ...
            <c:choose>
                <c:when test="${not empty reportList}">
                    <c:forEach var="report" items="${reportList}" varStatus="status">
                        <div class="table-row">
                            ...
                            <div class="col-title" onclick="location.href='${pageContext.request.contextPath}/report/detail?id=${report.id}'" style="cursor:pointer;">
                                <c:out value="${report.title}" />
                            </div>
                            <%-- 답변 여부에 따른 클래스 처리 --%>
                            <div class="col-answer ${report.answered ? 'complete' : ''}">
                                ${report.answered ? '답변 완료' : '답변 전'}
                            </div>
                            <div class="col-date">${report.regDate}</div>
                            <div class="col-manage">
                                <button class="delete-btn" onclick="if(confirm('신고 내역을 삭제하시겠습니까?')) location.href='${pageContext.request.contextPath}/report/delete?id=${report.id}'">삭제하기</button>
                            </div>
                        </div>
                    </c:forEach>

                    <%-- 최소 10줄을 유지하기 위한 빈 줄 채우기 --%>
                    <c:if test="${reportList.size() < 10}">
                        <c:forEach begin="1" end="${10 - reportList.size()}">
                ...
                <c:otherwise>
                    <%-- 데이터가 없을 때 --%>
                    <div class="table-row" style="justify-content: center; color: #999;">신고 내역이 없습니다.</div>
                    ...
                    <c:when test="${not empty totalPages && totalPages > 0}">
                        <c:forEach var="p" begin="1" end="${totalPages}">
                            <span class="page-num ${p == currentPage ? 'active' : ''}"
                                  onclick="location.href='${pageContext.request.contextPath}/members/mypage/myreport?page=${p}'"
